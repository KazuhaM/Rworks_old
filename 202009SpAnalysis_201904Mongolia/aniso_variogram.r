...
# site.name <- "W2_3"
  ...
  for (i_dir in 1:dir.len) {
    sill.conv <- c(0)
    # print(i_dir)
    d.aniso<- variogram(Z~X+Y,d,alpha=dir.list[i_dir], tol.hor = 90/dir.len)
    plot(d.aniso)
    spm.model.aniso<-vgm(psill=max(d.aniso$gamma)*0.5  ,
                         model="Exp",range=0.4, nugget=7.0 * 10^(-7)) #W2_3
    ...
    if (i_dir == dir.len) {
      dev.copy(pdf,
               file=paste("21forVariogram/anisovar_", site.name, ".pdf", sep = ""),
               width = 20, height = 10)
      dev.off()
    }
...
write.csv(result.all, "21forVariogram/anisovar_allresult2.csv")
