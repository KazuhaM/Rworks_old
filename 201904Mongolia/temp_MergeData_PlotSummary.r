...
# fig ---------------------------------------------------------------------
    ...
    matplot(d.ws.dist[,1],d.ws.dist[,2],xlab="", ylab="Temperature(K)",type="l", xaxt="n",
            ylim = c(min(d.ws.dist[,2:ncol(d.ws.dist)])*1.2,max(d.ws.dist[,2:ncol(d.ws.dist)])*1.2),col=1, main=dname)
    ...
    mtext(-0.0098*0.5, side = 2, line = 0, at = -0.8,cex = 0.6)
...
# time --------------------------------------------------------------------
    ...
    ## 昼を4に（赤色）(夜を2)
    timetest<-ymd_hms(d$Time,tz ="Asia/Ulaanbaatar")
