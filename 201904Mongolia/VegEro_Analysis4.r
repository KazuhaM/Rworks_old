...
plot(c ~ QuadCover, data = d3,col="Black",
     ...
     cex = 1,pch = 21, cex.axis = 1,cex.lab = 1.2,
     ylab = "",
     xlab = "cover (%)",xlim = c(0,45),ylim = c(0,8*10^-4),yaxt ="n")
...
dev.copy(cairo_pdf, file=paste(path4,"/regression_Ust_cSA4.pdf",sep=""), width = 9, height = 4.5)
dev.off()
...
plot(lambda ~ QuadCover, data = d3, bg = brewer.pal(10,"Spectral")[9 - round(d3$rh/20,0)],
     cex = 1,pch = 21, col = 1, cex.axis = 1,cex.lab = 1.2,
     ylab =expression(italic(lambda)),
     xlab = "cover (%)",xlim = c(0,45),ylim = c(0,0.15),yaxt = "n")
axis(side = 2, las=T,tck=-0.02)
legend("topright",title  =expression(italic(bar(D)/h)),rhlegendlab,
       pt.bg = brewer.pal(10,"Spectral")[max(round(d3$rh/20,0)):1],pch=21,
       cex = 1,col=1,ncol =2)
dev.copy(cairo_pdf, file=paste(path4,"/regression_lamCV_SA4.pdf",sep=""), width = 4.5, height = 4.5)
dev.off()
...
## 保存 
dev.copy(cairo_pdf, file=paste(path4,"/group_vegSA_sig4.pdf",sep=""), width = 9, height =9)
dev.off()
