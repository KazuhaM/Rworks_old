  ...
  for(i in 1 : length(d.flist300)){
    ...
    for(l in 1:nrow(eventperiod)){
      ...
      if(nrow(d.ev) !=0){
        ...
        d.ws <- d.ev[,c(1, grep("WS",colnames(d.ev)))]
        d.wd <- d.ev[,c(1, grep("WD",colnames(d.ev)))]
        ...
        #drifting sands
        plot(d.pc$Time,d.pc[,2],xlab="", ylab="Drifting Sands(n/s)",  main=paste(averate[k],dname,"event",d.ev$Event[1]),
