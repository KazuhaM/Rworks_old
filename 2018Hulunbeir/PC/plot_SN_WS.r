  ...
  for(l in c(6,18)){
    ...
    if(length(which((d4[,3] == lev[[3]][l])==TRUE))==0){
      ...
      if(length(d4[d4[,5]>= 1 & d4[,3] == lev[[3]][l] ,6])==0){
        ...
        d2 <- d2[!is.na(d2[,1]),]
        plot(d2$WindSpeed, d2$DriftSand, xlab ="Wind speed(m/s)",
             ylab ="Saltation number(n/s)",xlim=c(0,9),
             ylim = c(0,11),col = 4,cex.lab = 1.8,pch = 4,cex = 1.2)
        legend(0,10,c("site18 (cover = 55.5)","site6 (cover = 59.0)"),col = c(1,4),pch=c(1,4),cex = 1.5,)
