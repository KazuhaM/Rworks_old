...
if (a==1){
	yl<-"Erosion and Sedimentation (Erosion Pin)"
}else 
if (a == 2){
	yl<-"Litter Cover"
}else 
...
switch(b,               
   ...
   "2" = xl<-"Layer",
   "3" = xl<-"Treatment/Year",
   "4" = xl<-"Direction(only CT)",
   "5" = xl<-"Layer(only CT)"    
)

flname<-paste(year,A[a],"-",B[b], sep ="")
...
if(enna==4){
	...
	side=1,line=0,at=seq(1, enna*1, length=enna))
}else if(enna==3){
	mtext(paste("n=",echn,sep=""),cex=1.2, 
	side=1,line=0,at=seq(1, enna*1, length=enna))
}
